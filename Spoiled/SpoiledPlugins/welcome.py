...
import Spoiled.SqLDatabase.welcome_sql as sql
from config import DEV
from .helpers import (
    is_user_ban_protected,
    user_admin,
)
from .helpers import build_keyboard, revert_buttons
from .helpers import get_welcome_type
from .helpers import MessageHandlerChecker
from .helpers import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from Spoiled.SqLDatabase.global_bans_sql import is_user_gbanned
...
DEV_USERS = DEV.SUDO_USERS + [OWNER_ID]
...
async def send(update, message, keyboard, backup_message):
    ...
    if cleanserv:
        try:
            await CallbackContext.bot.delete_message(chat.id, update.message.message_id)
        except BadRequest:
            pass
        reply = False
    try:
...
async def new_member(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            # Welcome Devs
            if new_mem.id in DEV_USERS and new_mem.id != OWNER_ID:
                await update.effective_message.reply_text(
            ...
            # Welcome yourselflog
                ...
                creator = None
                async for x in bot.get_chat_administrators(update.effective_chat.id):
                    if x.status == "creator":
                        creator = x.user
                        break
                if creator:
        ...
        if (
            await is_user_ban_protected(chat, new_mem.id, chat.get_member(new_mem.id))
            or human_checks
        ):
            should_mute = False
